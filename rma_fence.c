...
int main(int argc, char** argv) {
    ...
    for (it = 0; it < maxit; it++) { //
        ...
        exchange_rma_fence_2d(a, wina, nx, sx, ex, sy, ey, nbrs[LEFT], nbrs[RIGHT], nbrs[UP], nbrs[DOWN]);
        //print_in_order(a,MPI_COMM_WORLD);
        sweep2d(a, f, nx, sx, ex, sy, ey, b);


        exchange_rma_fence_2d(b, winb, nx, sx, ex, sy, ey,  nbrs[LEFT], nbrs[RIGHT], nbrs[UP], nbrs[DOWN]);
    ...
    if (myid == 0) {
        write_grid("grid_outcomes_fence.txt", a, 0, nx + 1, nx, MPI_COMM_WORLD);
    }
