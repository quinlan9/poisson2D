...
//void Gather_grid_2d(double a[][maxn], int nx, int s0, int e0,int s1,int e1, MPI_Comm comm);
...
printf("myid:%d,coords[0]=%d,coords[1]=%d\n",myid,coords[0],coords[1]);
    ...
    MPI_Cart_shift(comm_cart, 1, 1, &nbrs[UP], &nbrs[DOWN]); //get the    of up and down of the current location
    
	MPE_Decomp2d(nx, dims,coords, myid,&sx,&ex,&sy,&ey);
//    MPE_Decomp2d(nx, dims, myid, &sx, &ex, &sy,&ey);//divied the area into sub block, each block assigned to specific myid
...
//printf("t1:%lf\n",t1);
    glob_diff = 1000; //global difference
    for (it = 0; it < maxit; it++) { //
//printf("1\n");

        exchange_nb_2d(a, nx, sx, ex,sy,ey, MPI_COMM_WORLD, nbrs[LEFT], nbrs[RIGHT], nbrs[UP], nbrs[DOWN]);
//print_in_order(a,MPI_COMM_WORLD);
        sweep2d(a, f, nx, sx, ex,sy,ey, b);


        exchange_nb_2d(b, nx, sx, ex, sy, ey, MPI_COMM_WORLD, nbrs[LEFT], nbrs[RIGHT], nbrs[UP], nbrs[DOWN]);
	...
	if (myid == 0) {
        write_grid("grid_outcomes_2d", a, 0, nx + 1, nx, MPI_COMM_WORLD);
